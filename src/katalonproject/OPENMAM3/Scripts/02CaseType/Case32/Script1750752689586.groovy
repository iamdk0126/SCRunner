...
'隨機讀取一筆未公開媒資名稱'
def specifickeyword = WebUI.callTestCase(findTestCase('05BaseTest/選擇未公開列管關鍵字'), [:], FailureHandling.CONTINUE_ON_FAILURE)


//def specifickeyword = CustomKeywords.'custom.dbHelper.getRandomAssetName'(0,1)
...
WebUI.callTestCase(findTestCase('05BaseTest/未公開媒資送播'), [:], FailureHandling.STOP_ON_FAILURE)

WebUI.callTestCase(findTestCase('05BaseTest/填寫送播原因'), [:], FailureHandling.CONTINUE_ON_FAILURE)

WebUI.callTestCase(findTestCase('05BaseTest/送播清單'), [:], FailureHandling.STOP_ON_FAILURE)

WebUI.callTestCase(findTestCase('05BaseTest/送播審核核准'), [('accountParam') : account,('keywordParam') : specifickeyword], FailureHandling.STOP_ON_FAILURE)
...
WebUI.closeBrowser()
*/
println('已完成 Case32 - 未公開媒資(列管送播-核准)')
WebUI.comment('已完成 Case32 - 未公開媒資(列管送播-核准)')
