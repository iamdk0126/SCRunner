...
//String dbFile = GlobalVariable.PTVDB_Folder + "/mxf_tasks.db"
String dbFile = GlobalVariable.CFDB_Folder + "/mxf_tasks.db"
MxfManager.setDbPath(dbFile)
//MxfManager.initCounterTable() // 確保序號表存在

// --- 3. 領取任務與處理 ---
// grabNextJob 會將該筆資料 status 設為 1 (使用中)，解決多機競爭問題
def job = MxfManager.grabNextJob(myRobotName)
int currentSerial = MxfManager.getNextSerialNumber() // 領取唯一流水號 (原子操作)
...
try {
	...
	if (job != null && currentSerial != -1) {
		...
		KeywordUtil.logInfo("🔢 目前執行流水號: ${currentSerial}")
    ...
    if (WebUI.verifyElementClickable(findTestObject('主畫面/未公開媒資頁/編輯媒資1'), FailureHandling.OPTIONAL)) {
		...
		WebUI.delay(2)
			...
			try {
				...
				try {
					long lastIndex = raf.length() - 1
					raf.seek(lastIndex)
            
					// --- 關鍵：紀錄原本的資料內容 ---
					originalByteValue = raf.read() 
					
					// --- 寫入新值 (讓 SHA256 變動) ---
					raf.seek(lastIndex)
					raf.write(currentSerial % 256)
					KeywordUtil.logInfo("SHA256 已微調，原始 Byte 值為: ${originalByteValue}")
				...
				while (elapsedSeconds < maxWait) {
					...
					if (!stillUploading) {
						...
						KeywordUtil.logInfo("任務完成並標記已結束: " + targetPath.toString())
				...
				// 4. 還原內容
				if (Files.exists(targetPath) && originalByteValue != -1) {
					RandomAccessFile rafCleanup = new RandomAccessFile(targetPath.toFile(), "rw")
					try {
						rafCleanup.seek(rafCleanup.length() - 1)
						rafCleanup.write(originalByteValue)
					} finally {
						rafCleanup.close()
					}
				}
				...
				KeywordUtil.logInfo("DB 任務 ID ${dbId} 狀態已重置為 0。")
			}
