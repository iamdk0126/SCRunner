...
// 找出該子目錄裡的所有 .mxf 檔案
List<File> mxfFiles = randomDir.listFiles().findAll { it.isFile() && it.name.toLowerCase().endsWith('.mxf') }

if (mxfFiles.isEmpty()) {
	KeywordUtil.markWarning("在子目錄 ${randomDir.name} 找不到任何 .mxf 檔案，流程暫停一小時。")
	WebUI.delay(ONE_HOUR_IN_SECONDS)
	return
}

// 隨機選取一個 .mxf 檔案
File randomMxfFile = mxfFiles[new Random().nextInt(mxfFiles.size())]
currentFilePath = randomMxfFile.getAbsolutePath()

KeywordUtil.logInfo("✅ 選取影片檔: ${currentFilePath}")
