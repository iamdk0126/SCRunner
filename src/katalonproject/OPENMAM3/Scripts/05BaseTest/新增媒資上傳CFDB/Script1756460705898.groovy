...
if (GlobalVariable.metateam == 2) {
		correspondingTitle = progtype +": "+ correspondingTitle +" 宣傳帶" 
	}
...
*/		
	...
	if (GlobalVariable.metateam == 2) {
		if (WebUI.verifyElementClickable(findTestObject('主畫面/未公開媒資頁/編輯媒資1'), FailureHandling.OPTIONAL)) {
			...
			TimecodeHelper.inputTimecode(findTestObject('主畫面/編輯媒資頁/timecode_起'), '00:00:00:00')
		
			String tapeValue = TimecodeHelper.getTimecode(findTestObject('主畫面/編輯媒資頁/時長'))
			
			TimecodeHelper.inputTimecode(findTestObject('主畫面/編輯媒資頁/timecode_迄'), tapeValue)
