...
String channel = "channel_8"

String recordsource = "VTR"  

String recordresolution = 'SD+HD' // SD+HD or SD

String recordmode = 'HD' // HD or SD

if (recordresolution.equalsIgnoreCase('SD+HD')) {recordmode = "HD"}
if (recordresolution.equalsIgnoreCase('SD')) {recordmode = "SD"}

String channelOption = channel+', '+recordsource+', '+recordresolution
...
if (result == null) {
	...
	Title = channel+" "+recordsource+" "+recordmode+" "+correspondingTitle
...
try {
	...
	//WebUIExtensions.retryClick(findTestObject('主畫面/新增媒資頁/建立媒資'))
	WebUIExtensions.clickTableTitle("節目-")
...
*/
	...
	WebUI.delay(2)
	//Title = channel+" "+recordsource+recordresolution+" " + correspondingTitle
	//WebUIExtensions.retrySetText(findTestObject('主畫面/編輯媒資頁/輸入Title'), correspondingTitle)
WebUIExtensions.setTextByLabel('媒資名稱', Title)
...
/*
	...
	WebUIExtensions.setTextByLabel('關鍵字', Title)
    ...
    if (WebUI.verifyElementClickable(findTestObject('主畫面/未公開媒資頁/編輯媒資1'), FailureHandling.OPTIONAL)) {
		...
		WebUI.callTestCase(findTestCase('05BaseTest/錄製影片VTR'), [('TitleParam') : Title, ('channelParam') : channelOption , ('resolutionParam') : recordresolution], FailureHandling.STOP_ON_FAILURE)
	...
	WebUI.delay(900)  //1分鐘後開始錄影 + 錄影時間 1分鐘 + 檔案上傳時間 暫訂30秒
