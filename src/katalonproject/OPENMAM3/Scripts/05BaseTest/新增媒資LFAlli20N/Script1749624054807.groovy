...
import java.io.FileFilter // 確保匯入了 FileFilter

// --- 日期準備工作 ---
LocalDate today = LocalDate.now()
DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMdd")
String formattedDate = today.format(formatter) // 例如 "0611"
...
for (int i = 1; i <= 20; i++) {
	...
	// 1. 動態產生 T001, T002 ... 這樣的後綴
    //    String.format("T%03d", i) 會將數字 i 格式化為3位數，不足的前面補零
    String suffix = String.format("T%03d", i)
    
    // 2. 組合出當次迴圈的 videoId
    String videoId = formattedDate + suffix // 例如第一次是 "0611T001", 第二次是 "0611T002"
