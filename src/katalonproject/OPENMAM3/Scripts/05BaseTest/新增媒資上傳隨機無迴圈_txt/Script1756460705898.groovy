...
CustomKeywords.'custom.TimeControl.checkPauseTime'(0, 0, 0, 20)  // 00:00 ~ 00:20 暫停
...
String allInfoFilePath = (BASE_DRIVE_S + formattedDate) + '-air.all.txt'

File allInfoFile = new File(allInfoFilePath)

WebUI.comment('正在檢查今日的 Video ID 檔案: ' + allInfoFilePath)

if (!(allInfoFile.exists()) || (allInfoFile.length() == 0)) {
    WebUI.comment('今日無可用 ID 資訊或檔案不存在，將在一小時後重試。')

    WebUI.delay(ONE_HOUR_IN_SECONDS) // continue
}

// --- 讀取 txt ---
List<String> rawAllInfoFileLines = new ArrayList()

try {
    allInfoFile.readLines('UTF-8').each({ def line ->
            def trimmedLine = line.trim()

            if (!(trimmedLine.isEmpty())) {
                rawAllInfoFileLines.add(trimmedLine)
            }
        })
}
catch (Exception e) {
    KeywordUtil.markWarning('讀取 Video ID 檔案時發生錯誤: ' + e.getMessage())

    WebUI.delay(ONE_HOUR_IN_SECONDS)
} //  continue

// --- 從 List 隨機選 Title 並檢查資料庫 ---
String specificVideoID = null

String correspondingTitle = null

while (!(rawAllInfoFileLines.isEmpty())) {
    int idx = randomGenerator.nextInt(rawAllInfoFileLines.size())

    String line = rawAllInfoFileLines.get(idx)

    def parts = line.split('\t', 2)

    if (parts.length < 2) {
        rawAllInfoFileLines.remove(idx) //    continue
    }
    
    String videoIdCandidate = (parts[0]).trim()

    String titleCandidate = (parts[1]).trim()
    ...
    try {
        ...
        conn = DriverManager.getConnection("jdbc:mysql://$GlobalVariable.mysqlip:3306/$GlobalVariable.mysqldbname?serverTimezone=UTC&useSSL=false", 
            GlobalVariable.mysqluser, GlobalVariable.mysqlpw)

        stmt = conn.createStatement()

        rs = stmt.executeQuery("SELECT 1 FROM assets WHERE asset_name = '$titleCandidate' LIMIT 1")
    ...
    if (!(existsInDb)) {
        specificVideoID = videoIdCandidate

        correspondingTitle = titleCandidate

        break
    } else {
        rawAllInfoFileLines.remove(idx)
    }
}

if (specificVideoID == null) {
    WebUI.comment('所有 Title 已存在於資料庫，將在一小時後重試。')
...
if (mxfFiles.isEmpty()) {
    KeywordUtil.markWarning("警告: 在目錄 $VIDEO_DIR_D 中找不到任何 .MXF 檔案! 流程將暫停一小時。")
...
try {
    ...
    WebUI.click(findTestObject('主畫面/新增媒資頁/建立媒資'), FailureHandling.OPTIONAL)
	...
	WebUI.setText(findTestObject('主畫面/編輯媒資頁/輸入Title'), correspondingTitle)
    ...
    if (WebUI.verifyElementClickable(findTestObject('主畫面/編輯媒資頁/儲存'), FailureHandling.OPTIONAL)) {
        WebUI.click(findTestObject('主畫面/編輯媒資頁/儲存'), FailureHandling.OPTIONAL)
    }
    ...
    WebUI.setText(findTestObject('主畫面/未公開媒資頁/輸入關鍵字'), correspondingTitle)
