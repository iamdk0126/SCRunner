...
WebUIExtensions.retryClick(findTestObject('回調清單頁/送播清單'))

WebUI.waitForElementVisible(findTestObject('回調清單頁/申請送播'), 2)
...
while (WebUI.verifyElementVisible(findTestObject('回調清單頁/關閉回調清單'),  FailureHandling.OPTIONAL) && closeAttempt < maxCloseAttempts) {
    ...
    try {
        // 1. 點擊 [申請送播] 按鈕 (第一次是觸發表單關閉，後續可能是重試觸發)
        // 注意：這裡假設每次點擊「申請送播」會嘗試關閉表單。
        // 如果「申請送播」只在表單未開啟時才有效，則需要調整邏輯。
        // 但根據您原有的程式碼，似乎點擊它就是為了觸發關閉。
        WebUIExtensions.retryClick(findTestObject('回調清單頁/申請送播'))
