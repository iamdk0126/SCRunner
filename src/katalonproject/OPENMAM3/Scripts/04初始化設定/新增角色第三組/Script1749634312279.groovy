...
List Team3BossList = GlobalVariable.Team3Boss

int Bosscount = Team3BossList.size()

List Team3LeaderList = GlobalVariable.Team3Leader

int Leadercount = Team3LeaderList.size()

List Team3UserList = GlobalVariable.Team3User

int Usercount = Team3UserList.size()
...
for (int i = 1; i <= count; i++) {
    ...
    if (i == 1) {
        for (int j = 1; j <= Bosscount; j++) {
            String currentBoss = Team3BossList.get(j - 1)
    ...
    if (i == 2) {
        for (int j = 1; j <= Leadercount; j++) {
            String currentLeader = Team3LeaderList.get(j - 1)
    ...
    if (i == 3) {
        for (int j = 1; j <= Usercount; j++) {
            String currentUser = Team3UserList.get(j - 1)
