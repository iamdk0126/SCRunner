...
WebUI.comment('Team =' + Team)

String TemplateName = Team + "TemplateMap"

WebUI.comment('TemplateName =' + TemplateName)


// 1. 獲取 TextType Map
//Map textTypeMap = GlobalVariable.Team1TextType // 改名並確認類型是 Map
Map templateMap = GlobalVariable.metaClass.getProperty(GlobalVariable, TemplateName) // 改名並確認類型是 Map

//int count = templateMap.size()



//def templateMap = GlobalVariable.Team1TemplateMap

String tapeParam = GlobalVariable.divapts_name
...
//WebUI.click(findTestObject('ResourceTeam/Team'))
...
for (int i = 1; i <= count; i++) {
	...
	if (currentType.equalsIgnoreCase('短帶')) {	
			DropdownHelper.selectDropdownOptionByLabel('媒資分類',specificvalue)	
			WebUIExtensions.setTextByLabel('媒資名稱', '短帶-')
			//WebUI.setText(findTestObject('新增模板/Page_/新增模板名稱'), '短帶-')
			DropdownHelper.selectDropdownOptionByLabel('類型',specificvalue)
			
			
			TimecodeHelper.inputTimecode(findTestObject('主畫面/編輯媒資頁/timecode_起'), '00:00:01:00')
			TimecodeHelper.inputTimecode(findTestObject('主畫面/編輯媒資頁/timecode_迄'), '00:00:30:00')		
					
	} else if (currentType.equalsIgnoreCase('節目')) {
			DropdownHelper.selectDropdownOptionByLabel('媒資分類',specificvalue)
			//WebUI.setText(findTestObject('新增模板/Page_/新增模板名稱'), '節目-')
			WebUIExtensions.setTextByLabel('媒資名稱', '節目-')
			
	} else {
			DropdownHelper.selectDropdownOptionByLabel('媒資分類',specificvalue)
			//WebUI.setText(findTestObject('新增模板/Page_/新增模板名稱'), currentType)
			WebUIExtensions.setTextByLabel('媒資名稱', currentType)
	}
	...
	WebUIExtensions.setTextByLabel('模板命名', currentType)
        
    //WebUI.setText(findTestObject('新增模板/Page_/新增模板名稱'), '新聞名稱必填')
	
	WebUI.click(findTestObject('新增模板/Page_/新增預設模板媒資分類下拉'))
	
	DropdownHelper.selectVisibleDropdownOptionScroll(specificvalue)
	
	WebUI.delay(2)
	
	WebUI.click(findTestObject('新增模板/Page_/div__ant-select-selector'))
	// 選擇磁帶類型
	DropdownHelper.selectVisibleDropdownOptionScroll(tapeParam)
