...
// --- 1. 初始化設定 ---
String targetOptionText = valueParam

def selectTrigger = findTestObject('系統設定/審核流程頁/順位一/selector')

def selectInput = findTestObject('系統設定/審核流程頁/順位一/selector_input')
...
String activeOptionXpath = '(//div[contains(@class, \'ant-select-item-option-active\') and not(contains(@style,\'display: none\'))])[4]' //在當前頁面第4個表單, 所以用[4]
...
for (int i = 0; i < maxTries; i++) {
    ...
    WebUI.delay(0.1 // 每次按鍵後短暫延遲 300 毫秒，給予反應時間
        // 每次按鍵後短暫延遲 300 毫秒，給予反應時間
        )
}
