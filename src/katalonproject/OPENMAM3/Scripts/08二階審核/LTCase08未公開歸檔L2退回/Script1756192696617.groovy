...
// --- 定義此特定執行要使用的帳號密碼 ---
String UserNo = GlobalVariable.TestUser8
...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
        ...
        if (specifickeyword != null) {
            ...
            if (l1Result != null) {
                ...
                WebUI.callTestCase(findTestCase('05BaseTest/歸檔送出退回'), [('accountParam') : specificAccount, ('keywordParam') : specifickeyword, ('endTimeParam') : endTime], FailureHandling.STOP_ON_FAILURE)
                WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
    ...
    for (def index : (0..runloop - 1)) {
        ...
        if (specifickeyword != null) {
            ...
            WebUI.callTestCase(findTestCase('05BaseTest/登入'), [('accountParam') : L2Account, ('passwordParam') : specificEncryptedPassword, ('domainParam') : specificDomain, ('nodeParam') : node], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/歸檔送出退回'), [('accountParam') : specificAccount, ('keywordParam') : specifickeyword, ('endTimeParam') : infiniteEndTime], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
        }
