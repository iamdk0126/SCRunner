...
// --- 定義此特定執行要使用的帳號密碼 ---
String UserNo = GlobalVariable.TestUser31
...
if (executionTimeInMinutes > 0) {
	...
	while (System.currentTimeMillis() < endTime) {
		...
		def specifickeyword = CustomKeywords.'custom.KeywordHelper.getSpecifickeyword'('05BaseTest/選擇未公開列管關鍵字')
		...
		//AuthKeywords.ensureLogin(specificAccount, specificEncryptedPassword, specificDomain, true)
		WebUI.callTestCase(findTestCase('05BaseTest/登入'), [('accountParam') : specificAccount, ('passwordParam') : specificEncryptedPassword,('domainParam') : specificDomain], FailureHandling.STOP_ON_FAILURE)
		WebUI.callTestCase(findTestCase('05BaseTest/送播審核退回'), [('accountParam') : specificAccount, ('keywordParam') : specifickeyword], FailureHandling.STOP_ON_FAILURE)
		WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
		WebUI.comment('已完成:未公開媒資(列管送播-退回)')
		} else {
