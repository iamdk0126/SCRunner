...
// --- 定義此特定執行要使用的帳號密碼 ---
String UserNo = GlobalVariable.TestUser4
...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
		...
		WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXF'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : iterationCount], FailureHandling.STOP_ON_FAILURE)
    ...
    for (def index : (0..runloop - 1)) {
		...
		WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXF'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : index], FailureHandling.STOP_ON_FAILURE)
