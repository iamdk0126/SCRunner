...
// --- 帳號與環境定義 ---
String UserNo = GlobalVariable.TestUser21
...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
        ...
        def specifickeyword = CustomKeywords.'custom.KeywordHelper.getSpecifickeyword'('05BaseTest/選擇公開無列管關鍵字')
        
        boolean result = WebUI.callTestCase(findTestCase('06二審CaseType/L2剪輯一鍵回調'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):endTime], FailureHandling.STOP_ON_FAILURE)
        ...
        if (result) {
            ...
            boolean l1Status = WebUI.callTestCase(findTestCase('05BaseTest/回調審核核准'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):endTime], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
            ...
            if (l1Status) {
                ...
                WebUI.callTestCase(findTestCase('05BaseTest/回調審核退回'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):endTime], FailureHandling.STOP_ON_FAILURE)
                WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
            } else {
    ...
    for (def index : (0..runloop - 1)) {
        ...
        boolean result = WebUI.callTestCase(findTestCase('06二審CaseType/L2剪輯一鍵回調'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):infiniteEndTime], FailureHandling.STOP_ON_FAILURE)
        WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
        
        if (result) {
            WebUI.callTestCase(findTestCase('05BaseTest/登入'), [('accountParam'):L1Account, ('passwordParam'):specificEncryptedPassword, ('domainParam'):specificDomain, ('nodeParam'):node], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/回調審核核准'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):infiniteEndTime], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
            
            WebUI.callTestCase(findTestCase('05BaseTest/登入'), [('accountParam'):L2Account, ('passwordParam'):specificEncryptedPassword, ('domainParam'):specificDomain, ('nodeParam'):node], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/回調審核退回'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):infiniteEndTime], FailureHandling.STOP_ON_FAILURE)
            WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
        }
