...
// --- 帳號與環境定義 ---
String UserNo = GlobalVariable.TestUser13
...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
        ...
        boolean result = WebUI.callTestCase(findTestCase('06二審CaseType/L2一鍵回調'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):endTime], FailureHandling.STOP_ON_FAILURE)
    ...
    for (def index : (0..runloop - 1)) {
        ...
        boolean result = WebUI.callTestCase(findTestCase('06二審CaseType/L2一鍵回調'), [('accountParam'):specificAccount, ('keywordParam'):specifickeyword, ('endTimeParam'):infiniteEndTime], FailureHandling.STOP_ON_FAILURE)
        WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
