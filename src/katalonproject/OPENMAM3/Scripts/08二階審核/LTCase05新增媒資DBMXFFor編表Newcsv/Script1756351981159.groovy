...
// --- 定義此特定執行要使用的帳號密碼 ---
String UserNo = GlobalVariable.TestUser5
...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
		...
		if (GlobalVariable.metateam == 1) {
			...
			progtype = "新聞"
				...
				specificAccount = "test2"+UserNo
				progtype = "節目"
				Title = Titles[new Random().nextInt(Titles.size())]
				if (Title == "看公視說英語") {VideoID = "82"} 
				  else if (Title == "看公視學日語") {VideoID = "0000ABC"}
				  else if (Title == "看公視做運動") {VideoID = "0000CBA"}
				  else if (Title == "看公視聊法律") {VideoID = "110"}
				  else if (Title == "看公視當大廚") {VideoID = "452"}
			}

		
		def correspondingTitle = WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXFFor編表csv'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : iterationCount], FailureHandling.STOP_ON_FAILURE)
		...
		if (GlobalVariable.metateam == 2) {
			...
			WebUI.callTestCase(findTestCase('05BaseTest/媒資timecode抓取'),[:], FailureHandling.STOP_ON_FAILURE)
    ...
    for (def index : (0..runloop - 1)) {
		...
		if (GlobalVariable.metateam == 1) {
			...
			progtype = "新聞"
				...
				if (Title == "看公視說英語") {VideoID = "82"} 
				 ...
				 tabM = FromCSV.getValue(csvname, rowIndex, 'M') //節目編號
				 tabN = FromCSV.getValue(csvname, rowIndex, 'N') //節目名稱
				 tabAE = FromCSV.getValue(csvname, rowIndex, 'AE') //集數
				 Title = tabN
				 VideoID = tabM
		...
		def correspondingTitle = WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXFFor編表csv'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : index,('rowIndexParam') : rowIndex], FailureHandling.STOP_ON_FAILURE)
