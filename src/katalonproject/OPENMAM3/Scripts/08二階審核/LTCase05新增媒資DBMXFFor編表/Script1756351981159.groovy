...
if (executionTimeInMinutes > 0) {
    ...
    while (System.currentTimeMillis() < endTime) {
		...
		def correspondingTitle = WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXFFor編表'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : iterationCount], FailureHandling.STOP_ON_FAILURE)
    ...
    for (def index : (0..runloop - 1)) {
		...
		def correspondingTitle = WebUI.callTestCase(findTestCase('05BaseTest/新增媒資上傳同MXFFor編表'), [('titleParam') : Title, ('videoParam') : VideoID,('progParam') : progtype, ('indexParam') : index], FailureHandling.STOP_ON_FAILURE)
