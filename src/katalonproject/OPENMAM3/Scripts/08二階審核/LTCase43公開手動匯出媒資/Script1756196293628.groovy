...
// --- 定義此特定執行要使用的帳號密碼 ---
String UserNo = GlobalVariable.TestUser43
...
if (executionTimeInMinutes > 0) {
	...
	while (System.currentTimeMillis() < endTime) {
		...
		WebUI.callTestCase(findTestCase('05BaseTest/匯出媒資'), [('accountParam') : specificAccount, ('keywordParam') : specifickeyword], FailureHandling.STOP_ON_FAILURE)
		
		WebUI.callTestCase(findTestCase('05BaseTest/登出'), [:], FailureHandling.STOP_ON_FAILURE)
		
		WebUI.comment('已完成:公開媒資(匯出媒資)')
