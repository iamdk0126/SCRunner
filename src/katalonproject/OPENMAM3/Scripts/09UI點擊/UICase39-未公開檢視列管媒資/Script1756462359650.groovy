...
if (executionTimeInMinutes > 0) {
	...
	while (System.currentTimeMillis() < endTime) {
		...
		String keywordStr = specifickeyword
		
		if (keywordStr != null && keywordStr.length() > 0) {
			
			WebUI.callTestCase(findTestCase('05BaseTest/未公開檢視媒資資訊'), [('keywordParam') : specifickeyword], FailureHandling.STOP_ON_FAILURE)
			
			} else {
				println "Keyword 為空，跳過步驟。"
			}
